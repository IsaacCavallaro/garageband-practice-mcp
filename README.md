...
V1 creates a per-song workspace, imports authorized MIDI, searches Mutopia Project public-domain/open-license results, generates piano grand-staff MusicXML charts, optionally exports PDF/PNG through a local MuseScore install, and opens the MIDI handoff in GarageBand.
...
4. Run `generate_piano_grand_staff_chart` and `generate_browser_practice_view`.
5. Run `open_in_garageband` to open the active MIDI handoff in GarageBand.
6. In GarageBand, select the playalong/software-instrument track, confirm the MIDI keyboard and audio interface are active, then save or export any GarageBand-specific work.
7. Run `save_session_snapshot` with `commit: true` and `push: true` when the song artifacts should be committed and pushed.
...
4. Generate the browser practice view when you want score reading and local MIDI-derived playback side by side.
5. Open the active MIDI in GarageBand.
6. Save the `.band` project manually from GarageBand if desired.
...
## Later Hybrid KeyLab Phase

The later phase should add a local browser practice view using Web MIDI for keyboard input, Tone.js for scheduled playalong, and VexFlow for notation rendering. That feedback surface belongs outside GarageBand because GarageBand is not a reliable programmable live-scoring UI.
